...
namespace KJC.IMS.ENTR.StaffMngr
{
    ...
    public partial class ApplMajorGuideMsgMngr2 : WebFormBase
    {
        #region 초기화
        ...
        {
            string spName = "dbo.USP_학사행정_입시_지원자현황_지원자안내관리_등록_업그레이드2";
            ...
            try
            {
                ...
                if (shell.ErrorCode == 0)
                {
                    ...
                    spName = "dbo.USP_학사행정_입시_지원자현황_지원자안내관리_조회_업그레이드2";
                ...
                foreach (GridViewRow item in grdList.Rows)
                {
                    ...
                    if (COMMCommon.IsDataItem(item.RowType))
                    {
                        if (((CheckBox)item.Cells[4].Controls[1]).Checked)
                        {
                            ...
                            parameters.Add("@Year", item.Cells[5].Text);
                            parameters.Add("@Season", item.Cells[6].Text);
                            parameters.Add("@PassCd", item.Cells[7].Text);
        ...
        {
            string spName = "dbo.USP_학사행정_입시_지원자현황_지원자안내관리_조회_업그레이드2";
            var parameters = new DataParameterCollection();
            var shell = new DataCommandShell();
            var dataCommands = new List<DataCommand>();
        ...
        private void SelectItem(GridViewRow gvr)
        {
            try
            {
                txtApplyYear.Text = Util.GetGridViewString(gvr.Cells[5].Text);
                ddlApplySeason.SelectedValue = Util.GetGridViewString(gvr.Cells[6].Text);
                ddlPassCode.SelectedValue = Util.GetGridViewString(gvr.Cells[7].Text);
                ...
                parameters.Add("@Year", gvr.Cells[5].Text);
                parameters.Add("@Season", gvr.Cells[6].Text);
                parameters.Add("@PassCd", gvr.Cells[7].Text);
