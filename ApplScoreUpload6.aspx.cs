...
{
    public partial class ApplScoreUpload6 : WebFormBase
    {
    #region 초기화
    ...
    public override void SaveCmd(object sender, CommandEventArgs e)
    {
        try
        {
            ...
            string spName = "dbo.USP_학사행정_입시_성적사정_고교학생부입력2007입시이전_성적_파일등록_업그레이드";
                ...
                */
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    if (iIdx <= rowsCount)
                    {
                        ...
                        string Rank = Util.GetDataRowString(dr[6]);
                        string StudentCount = Util.GetDataRowString(dr[7]);

                            if (string.IsNullOrEmpty(Year) || string.IsNullOrEmpty(Grade) || string.IsNullOrEmpty(Term) || string.IsNullOrEmpty(OrganizationName) || string.IsNullOrEmpty(SubjectName) || string.IsNullOrEmpty(Rank) || string.IsNullOrEmpty(StudentCount) )
                        {
                ...
                foreach (DataRow dr in dt.Rows)
                {
                    if (iIdx <= rowsCount)
                    {
                        ...
                        parameters.Add("@StudentCount", StudentCount);
    ...
    private void Retrieve()
    {
        try
        {
            string spName = "dbo.USP_학사행정_입시_성적사정_고교학생부입력2007입시이전_성적_파일조회_업그레이드";
            var parameters = new DataParameterCollection();
            var shell = new DataCommandShell();
            var dataCommands = new List<DataCommand>();
