...
namespace KJC.IMS.ENTR.StaffMngr
{
	...
	public partial class highClassGradeList4 : WebFormBase
	{
		#region 초기화
		...
		public override void Etc1Cmd(object sender, CommandEventArgs e)
		{
			try
			{
                ...
                {
                    sp_Name = "dbo.USP_학사행정_입시_통계_지역별지원자인원현황_조회_업그레이드";
                }
                else if (rdGbn.SelectedValue == "02")               // 합격자 
                {
                    sp_Name = "dbo.USP_학사행정_입시_통계_지역별합격자인원현황_조회_업그레이드";
                }
                else                                                // 등록자
                {
                    sp_Name = "dbo.USP_학사행정_입시_통계_지역별등록자인원현황_조회_업그레이드";
                }

                rv1.ShowReportByStoredProcedure("0001508001", sp_Name, dataParams);   // rptHighClassGradeList4
