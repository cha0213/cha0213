...
namespace KJC.IMS.ENTR.StaffMngr
{
    ...
    public partial class highSchoolScoreInput5 : WebFormBase
    {
        #region 전역변수
        ...
        {
            string spName = "dbo.USP_학사행정_입시_성적사정_고교학생부입력2021입시이후_성적_삭제_업그레이드";
        ...
        {
            DataSet ds = null;
            string spName = "dbo.USP_학사행정_입시_성적사정_고교학생부입력2021입시이후_출석_조회_업그레이드"; //2007학년이전 프로시저와 동일
            var parameters = new DataParameterCollection();
            var shell = new DataCommandShell();
            var dataCommands = new List<DataCommand>();
        ...
        {
            DataSet ds = null;
            string spName = "dbo.USP_학사행정_입시_성적사정_고교학생부입력2021입시이후_성적_조회_업그레이드"; //2007학년이전 프로시저와 동일
            var parameters = new DataParameterCollection();
            var shell = new DataCommandShell();
            var dataCommands = new List<DataCommand>();
        ...
        private void SaveAttendance()
        {
            ...
            if (this.txtAbsence_Disease_1.Text.ToString().Trim() == "") absence_Disease_1 = 0;
            ...
            try
            {
                string spName = "dbo.USP_학사행정_입시_성적사정_고교학생부입력2021입시이후_출석_등록_업그레이드";
                var parameters = new DataParameterCollection();
                var shell = new DataCommandShell();
                var dataCommands = new List<DataCommand>();
        ...
        private void SaveScore()
        {
            ...
            */
            ...
            string ScoreYear = txtScoreYear.Text.Trim(); //연도
            string Grade = txtGrade.Text.Trim(); //학년
            int Term = Int32.Parse(txtTerm.Text.Trim()); //학기
            string OrganizationCode = txtOrganizationCode.Text.Trim(); //편제코드
            string OrganizationName = txtOrganizationName.Text.Trim(); //편제명
            string CourceCode = txtCourceCode.Text.Trim(); //교과코드
            string CourceName = txtCourceName.Text.Trim(); //교과명
            string SubjectCode = txtSubjectCode.Text.Trim(); //과목코드

            OrganizationName = "XXX"; //편제명
            OrganizationCode = "XXX"; //편제코드
            CourceCode = "XXX"; //교과코드



            string SubjectName = txtSubjectName.Text.Trim(); //과목명
            string Unit = txtUnit.Text.Trim(); //단위수
            string OriginalScore = txtOriginalScore.Text.Trim(); //원점수
            string AvgScore = txtAvgScore.Text.Trim(); //평균점수
            string StandardDeviation = txtStandardDeviation.Text.Trim(); //표준편차
            string RankingGrade = txtRankingGrade.Text.Trim(); //등급
            int SeqNumber = Int32.Parse(txtSeqNumber.Text.Trim().Length == 0 ? "0" : txtSeqNumber.Text.Trim());

            #endregion

            try
            {
                string spName = "dbo.USP_학사행정_입시_성적사정_고교학생부입력2021입시이후_성적_등록_업그레이드";
                var parameters = new DataParameterCollection();
                var shell = new DataCommandShell();
                var dataCommands = new List<DataCommand>();

                #region 파라미터 설정
                parameters.Add("@year", year); //연도
                parameters.Add("@recpNo", recpNo); //학번
                parameters.Add("@ScoreYear", "0000"); //성적연도
                parameters.Add("@Grade", Grade); //학년
                parameters.Add("@OrganizationCode", OrganizationCode); //편제코드
                parameters.Add("@OrganizationName", OrganizationName); //편제명
                parameters.Add("@CourceCode", CourceCode); //교과코드
                parameters.Add("@CourceName", CourceName); //교과명
                parameters.Add("@SubjectCode", SubjectCode); //과목코드
                parameters.Add("@SubjectName", SubjectName); //과목명
                parameters.Add("@Term", Term); //학기
                parameters.Add("@Unit", Unit); //단위수
                parameters.Add("@OriginalScore", OriginalScore); //원점수
                parameters.Add("@AvgScore", AvgScore); //평균
                parameters.Add("@StandardDeviation", StandardDeviation); //표준편차
                parameters.Add("@RankingGrade", RankingGrade); //등급
                parameters.Add("@SeqNumber", SeqNumber); //순번
                parameters.Add("@ProcessID", UserId);
                parameters.Add("@ProcessIP", UserIp);
                ...
                else
                {
                    switch (shell.ErrorCode)
                    {
                        ...
                        case 2627:
                            ...
                            break;
                        default:
                            CommonMessage.AlertMessage(this, "전산소로 문의(미확인 오류)");
                            break;
                    }
