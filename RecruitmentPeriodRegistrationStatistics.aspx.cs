...
namespace KJC.IMS.ENTR.StaffMngr
{
    ...
    public partial class RecruitmentPeriodRegistrationStatistics : WebFormBase
    {
        #region 초기화
        ...
        public override void Etc1Cmd(object sender, CommandEventArgs e)
        {
            try
            {
                ...
                {
                    rv1.ShowReportByStoredProcedure("0001514002", "dbo.USP_학사행정_입시_통계_모집시기별등록률_조회_업그레이드", dataParams);
                }
                else
                {
                    rv1.ShowReportByStoredProcedure("0001514001", "dbo.USP_학사행정_입시_통계_모집시기별등록률_조회_업그레이드", dataParams);   // rptRecruitmentPeriodRegistrationStatistics
                }
